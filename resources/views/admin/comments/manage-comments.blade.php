                ...
                <table id="datatable1" class="table-responsive table display responsive" style="width: 100%;">
                    ...
                    @foreach($comments as $comment)
                        ...
                        @if($comment->status==1)
                            ...
                            <td>
                                @if($comment->status==1)
                                <a href="{{route('change-comment-status',['id'=>$comment->id])}}" onclick="confirm('Are you sure to change status of the comment?')"
                            ...
                            <td>
                                ...
                                <a href="{{route('change-client-status',['id'=>$comment->client_id])}}" class="btn btn-sm btn-danger"  onclick="confirm('Are you sure to change status of the user?')">Ban User</a>
                                @else
                                    <a href="{{route('change-client-status',['id'=>$comment->client_id])}}"  class="btn btn-sm btn-success"  onclick="confirm('Are you sure to change status of the user?')">Unban User</a>
                                @endif
                            </td>
